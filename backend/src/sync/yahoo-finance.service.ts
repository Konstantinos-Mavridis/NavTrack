...
export class YahooFinanceService {
   ...
   * @param fromDate  YYYY-MM-DD, defaults to 5 years ago
  ...
  async fetchHistory(
    ...
    const from = fromDate ? new Date(fromDate) : fiveYearsAgo;
    const to   = toDate   ? new Date(toDate)   : now;

    if (from.getTime() > to.getTime()) {
      this.logger.warn(
        `Skipping history fetch for ${symbol}: invalid range ${fromDate ?? '5y ago'} > ${toDate ?? 'today'}`,
      );
      return [];
    }
    ...
    this.logger.log(`Fetching history for ${symbol} from ${fromDate ?? '5y ago'} to ${toDate ?? 'today'}`);
