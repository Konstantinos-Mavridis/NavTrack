...
function validateIsoDate(date: string | undefined, fieldName: string): void {
  if (date && !/^\d{4}-\d{2}-\d{2}$/.test(date)) {
    throw new BadRequestException(`${fieldName} must be in YYYY-MM-DD format, got: ${date}`);
  }
}
...
export class SyncService {
  ...
  async syncInstrument(
    ...
    // Validate date inputs before using them in SQL or storage
    validateIsoDate(opts.fromDate, 'fromDate');
    validateIsoDate(opts.toDate, 'toDate');
    ...
    try {
      ...
      if (!ticker) {
        ...
        if (ticker) {
          ...
          // Use parameterized query with jsonb_build_object to prevent SQL injection.
          await this.instrumentRepo
            .createQueryBuilder()
            .update(Instrument)
            .set({
              externalIds: () =>
                `external_ids || jsonb_build_object('yahoo_ticker', CAST(:ticker AS text))`,
            })
      ...
      // 3. Fetch from Yahoo Finance using explicit UTC handling.
      // Use effectiveToDate for both the guard check AND the actual fetch call.
      // Always work with UTC dates (YYYY-MM-DD format) to ensure consistent
      // behavior across timezones. toISOString() returns UTC, so slice(0, 10)
      // gives us the UTC date. This prevents today's candle from being
      // excluded when the server is in a different timezone than UTC.
      const utcNow = new Date();
      const effectiveToDate = opts.toDate ?? utcNow.toISOString().slice(0, 10);
      ...
      // Pass effectiveToDate (not opts.toDate) so today's candle is always included in UTC.
      ...
      // 4. Batch upsert all nav_prices in a single query.
      // This is much more efficient than N individual upserts.
      // Dates are already in UTC (YYYY-MM-DD), so no timezone conversion needed.
      if (points.length > 0) {
        await this.navRepo
          .createQueryBuilder()
          .insert()
