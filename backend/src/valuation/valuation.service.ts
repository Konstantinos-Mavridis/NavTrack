...
export interface ValuationResult {
  portfolioId: string;
  date: string;
  /** The most recent date for which at least one instrument in this portfolio has a NAV price. */
...
export class ValuationService {
  ...
  async compute(portfolioId: string, date?: string): Promise<ValuationResult> {
    ...
    // Determine the latest NAV date across all instruments in this portfolio
    const instrumentIds = positions.map((p) => p.instrumentId);
    let latestNavDate: string | null = null;

    if (instrumentIds.length > 0) {
      const latestMap = await this.navPrices.latestForManyInstruments(instrumentIds);
      for (const nav of latestMap.values()) {
        const d = typeof nav.date === 'string' ? nav.date : (nav.date as Date).toISOString().slice(0, 10);
        if (latestNavDate === null || d > latestNavDate) latestNavDate = d;
      }
    }

    // If no explicit date is requested, anchor to the latest available NAV date
    const today = date ?? latestNavDate ?? new Date().toISOString().slice(0, 10);
