...
export class PortfoliosService {
  ...
  async aggregateValuationSeries(
    ...
    // Build an optional portfolio-filter clause using QueryBuilder.
    // We pass the IDs as parameterized values so user-supplied strings
    // never touch the SQL text itself — prevents SQL injection.
    ...
    if (toDate) {
      ...
      // Use QueryBuilder instead of raw SQL to prevent injection via template strings.
      let query = this.dataSource
        .getRepository('nav_prices')
        .createQueryBuilder('n')
        .select('MAX(n.date)', 'd')
        .innerJoin('transactions', 't', 't.instrument_id = n.instrument_id');

      if (hasIdFilter) {
        query = query.where('t.portfolio_id = ANY(:portfolioIds)', { portfolioIds: ids });
      }

      const result = await query.getRawOne<{ d: Date | string | null }>();
      const raw = result?.d;
      if (raw) {
    ...
    // For fixed ranges clamp to [2, 3650] to prevent absurd windows.
    // For ALL (days === undefined) the SQL itself uses MIN(trade_date) as
    // start_day, so no day arithmetic is needed.
      ...
      WITH bounds AS (
        SELECT
          CASE
            ...
            ELSE
              ($2::date - ($1::int - 1) * interval '1 day')::date
          END AS start_day,
          $2::date AS end_day
      ),
      days AS (
  ...
  async importPortfoliosFromCsv(csv: string): Promise<ImportSummary> {
    ...
    for (const row of rows) {
      // Validate only truly required columns are non-empty.
      // Optional fields such as portfolio_description, settlement_date,
      // instrument_name, and notes may legitimately be blank.
      const emptyCells = required.filter(
        (k) => typeof row[k] === 'string' && !row[k].trim(),
      );
      
      if (emptyCells.length > 0) {
        throw new BadRequestException(`CSV row has empty required columns: ${emptyCells.join(', ')}`);
      }
...
/**
 * Allowed column names for portfolio CSV imports.
 * Using an explicit allowlist prevents user-supplied header strings from
 * becoming arbitrary object property names, which would open the door to
 * prototype-pollution attacks (e.g. __proto__, constructor).
 */
...
function parseCsv(csv: string): Array<Record<string, string>> {
  ...
  // Sanitise header names against the allowlist before using them as object
  // keys.  Any column name not in the allowlist is replaced with a safe
  // placeholder so it can never shadow prototype-chain properties or inject
  // unexpected keys into downstream logic.
  const headers = rows[0].map((h, idx) => {
    const name = h.trim();
    return ALLOWED_CSV_HEADERS.has(name) ? name : `__unknown_col_${idx}__`;
  });

  // Use Object.create(null) so the resulting objects have no prototype and
  // are therefore immune to __proto__ / constructor pollution regardless of
  // what key names slip through.
