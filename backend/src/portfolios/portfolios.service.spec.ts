...
describe('PortfoliosService', () => {
  ...
  describe('importPortfoliosFromCsv', () => {
    ...
    it('accepts CSV when optional headers are omitted', async () => {
      instrumentRepo.find.mockResolvedValue([
        { id: 'inst-1', isin: 'IE0001234567', name: 'Test Fund' },
      ]);

      const header = [
        'portfolio_name,instrument_isin,type,trade_date,units,price_per_unit,fees',
      ].join('');
      const row = [
        'My Portfolio,IE0001234567,BUY,2024-01-15,100,10.00,0',
      ].join('');
      const csv = `${header}\n${row}`;
