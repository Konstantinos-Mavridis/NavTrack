...
/**
 * Validates and parses ISO date strings (YYYY-MM-DD).
 * Checks both format AND validity (e.g., 2026-13-45 fails).
 * Throws ApiError if invalid.
 */
function validateIsoDate(date: string | undefined, fieldName: string): void {
  if (!date) return;

  // Check format first
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
    throw new ApiError(400, `${fieldName} must be in YYYY-MM-DD format, got: ${date}`);
  }

  // Validate the date is actually valid (e.g., not 2026-13-45)
  const d = new Date(`${date}T00:00:00Z`);
  if (isNaN(d.getTime())) {
    throw new ApiError(400, `${fieldName} is not a valid date: ${date}`);
  }

  // Ensure the parsed date matches the input (catches edge cases like 2026-02-30)
  const parsed = d.toISOString().slice(0, 10);
  if (parsed !== date) {
    throw new ApiError(400, `${fieldName} is not a valid date: ${date}`);
  }
}
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  try {
    ...
    throw new ApiError(0, `Request failed: ${errorMsg}`, error instanceof Error ? error : undefined);
  }
}
...
export const api = {
  ...
  instruments: {
     ...
     * Validates date format and validity before making request.
     */
    navOnDate:  (id: string, date: string)       => {
      validateIsoDate(date, 'date');
      return request<NavPrice | null>(`/instruments/${id}/nav/on-date?date=${encodeURIComponent(date)}`);
    },
  ...
  portfolios: {
    ...
    aggregateSeries: (days?: number, toDate?: string, ids?: string[]) => {
      ...
      validateIsoDate(toDate, 'toDate');
      return request<AggregatePortfolioValuePoint[]>(
  ...
  valuation: {
    get: (portfolioId: string, date?: string) => {
      ...
      validateIsoDate(date, 'date');
      return request<ValuationResult>(`/portfolios/${portfolioId}/valuation${date ? `?date=${date}` : ''}`);
    },
  },
  ...
  templates: {
    ...
    navPreview: (id: string, tradeDate: string)  => {
      validateIsoDate(tradeDate, 'tradeDate');
      return request<TemplateNavPreview>(`/templates/${id}/nav-preview?tradeDate=${encodeURIComponent(tradeDate)}`);
    },
    ...
    navSeries: (id: string, range: TemplateRange, customFrom?: string, customTo?: string) => {
      if (range === 'CUSTOM') {
        validateIsoDate(customFrom, 'customFrom');
        validateIsoDate(customTo, 'customTo');
        ...
        throw new ApiError(400, 'CUSTOM range requires both customFrom and customTo dates');
      }
