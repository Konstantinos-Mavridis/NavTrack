...
export default function TemplateBuyModal({ portfolioId, valuationDate, onSaved, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    api.templates.navPreview(templateId, tradeDate)
      .then((result) => { if (active) setNavPreview(result); })
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-5">
        {error && (
          <div className="rounded-lg bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-700 text-red-700 dark:text-red-400 text-sm px-3 py-2">
            {error}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Total Amount (EUR) *</label>
            <input
        ...
        {selected && (
          <div className="rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <div className="px-3 py-2 space-y-2">
              {navLoading && (
                <p className="text-sm text-gray-500 dark:text-gray-400">Loading NAV values for {tradeDate}...</p>
              )}
              {!navLoading && navError && (
                <p className="text-sm text-red-600 dark:text-red-400">{navError}</p>
              )}
                    ...
                    <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                      {item.nav != null ? (
                        <>
                          ...
                          {fallback && (
                            <span className="text-amber-600 dark:text-amber-400"> — latest available on/before valuation date</span>
                          )}
                          {estUnits != null && (
              ...
              {!navLoading && !navError && previewItems.length === 0 && (
                <p className="text-sm text-gray-500 dark:text-gray-400">No template items found.</p>
              )}
