...
export default function PortfolioImportExport({ onImported }: Props) {
  ...
  /**
   * After a successful import, recalculate positions for every newly-created
   * portfolio so they reflect the imported transactions immediately.
   *
   * Strategy: fetch the full portfolio list, then fire recalculate on each one
   * in parallel. This is safe because the import always creates *new* portfolios
   * (the service never merges into an existing one), so recalculating all
   * portfolios cannot corrupt pre-existing data.
   */
  async function recalculateAll() {
    try {
      ...
      // Recalculation errors are non-fatal — the user can trigger it manually.
    }
  }
  ...
  async function handleImportJson(file: File) {
    ...
    try {
      ...
      if (result.portfoliosImported > 0) {
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 overflow-hidden">
            ...
            <div className="px-6 py-5 space-y-6">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mb-3">Portfolios are created fresh; positions are recalculated automatically from the imported transactions.</p>
                <div className="grid grid-cols-2 gap-3">
                  <button
