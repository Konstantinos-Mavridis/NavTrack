...
export default function TemplatePerformanceChart({ templateId, paused }: Props) {
  ...
  // Bounds for the date pickers — populated once on mount.
  ...
  // Fetch available range once on mount (used to bound the date pickers).
  useEffect(() => {
    if (paused) return;
    api.templates.navSeriesAvailableRange(templateId)
      .then((r) => {
        setAvailable(r);
        // Pre-fill the custom inputs to the full available range.
        setCustomFrom(r.from);
        setCustomTo(r.to);
      })
      .catch(() => { /* silently ignore — custom picker just won't be bounded */ });
  }, [templateId, paused]);

  // Fetch series whenever range / custom dates change.
  const load = useCallback(async () => {
    if (paused) return;
    // For CUSTOM, require both dates to be valid before fetching.
    if (range === 'CUSTOM') {
  ...
  function applyCustom() {
    // Trigger a fresh load by bumping the range — load() already depends on customFrom/To.
    setRange('CUSTOM');
    setShowCustom(false);
    load();
  }
    ...
    <div className="mb-4">
      ...
      <div className="flex items-center justify-between mb-2 flex-wrap gap-y-2">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {/* Custom range label when active */}
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
              ...
              <XAxis
                ...
                tickFormatter={(d: string) => formatShortDate(d, isLong)}
...
/** DD/MM/YYYY for tooltips and labels. */
...
/** MM/YY for long ranges (1Y, ALL, CUSTOM), DD/MM for short ones. */
function formatShortDate(date: string, long: boolean): string {
  const [year, month, day] = date.split('-').map(Number);
  if (!year || !month || !day) return date;
