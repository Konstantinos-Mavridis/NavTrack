...
export default function PortfolioList() {
  ...
  useEffect(() => {
    if (rows.length > 0) {
      void loadAggregateSeries(selectedRange);
    }
      ...
      {rows.length === 0 ? (
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          {rows.map((row) => {
            ...
            return (
              <div key={row.id} className="card p-6 hover:shadow-md hover:border-blue-100 transition-all group relative">
                ...
                <Link to={`/portfolios/${row.id}`} className="block">
                  ...
                  {row.valLoading ? (
                    ...
                    <p className="text-xs text-red-400">{row.valError}</p>
                  ) : v ? (
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-xs text-gray-400 uppercase tracking-wide">Total Value</p>
                        <p className="text-xl font-bold text-gray-900 mt-0.5">€{fmtEur(v.totalValue)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-400 uppercase tracking-wide">Unrealised P&L</p>
                        <p className={`text-xl font-bold mt-0.5 ${pnlPositive ? 'text-emerald-600' : 'text-red-500'}`}>
                          {pnlPositive ? '+' : ''}€{fmtEur(v.unrealisedPnl)}
                        </p>
                        <p className={`text-xs mt-0.5 ${pnlPositive ? 'text-emerald-500' : 'text-red-400'}`}>
                          {fmtPct(v.unrealisedPnlPct)}
                        </p>
                      </div>
                      <div className="col-span-2 mt-1">
                        <p className="text-xs text-gray-400 mb-1">{v.positions.length} position{v.positions.length !== 1 ? 's' : ''}</p>
                    ...
                    <p className="text-xs text-gray-400">No positions yet — add transactions to get started.</p>
                  )}
                </Link>
              </div>
            );
