...
import { fmtEur, fmtUnits } from '../utils/format';
...
export default function PortfolioDetail() {
  ...
  async function reload() {
    ...
    const [p, v, t] = await Promise.all([
      ...
      api.valuation.get(id),          // no date → backend picks latestNavDate
      api.transactions.list(id),
    ]);
    setPortfolio(p);
    setValuation(v);
    setDate(v.date);                  // anchor to the actual date returned
    setTransactions(t);
  }
  ...
  // Fetch valuation whenever `date` changes (debounced 300 ms)
  const fetchValuation = useCallback(async (targetDate: string) => {
    if (!id || !targetDate) return;
    setRefreshing(true);
    try {
      setValuation(await api.valuation.get(id, targetDate));
  ...
  async function autoRecalculate(portfolioId: string, valuationDate: string) {
    try {
      ...
      setValuation(await api.valuation.get(portfolioId, valuationDate));
    } catch {}
  }
  ...
  // Cap the date picker at the latest date for which we actually have NAV data
  const maxDate = valuation.latestNavDate ?? date;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
      ...
      {!hasPositions && (
        <div className="card p-8 text-center border-dashed dark:border-gray-700">
          ...
          <div className="flex justify-center gap-2 flex-wrap">
            <button onClick={() => setModal({ type: 'addTxn' })} className="btn-primary">
              + Add First Transaction
            </button>
            <button onClick={() => setModal({ type: 'buyTemplate' })} className="btn-secondary">
              + Buy Template
            </button>
          </div>
        </div>
      ...
      <div className="card overflow-hidden">
        ...
        {tab === 'transactions' && (
          <div className="flex justify-end px-4 py-3 border-t border-gray-100 dark:border-gray-800">
            <PortfolioImportExport
              onImported={() => {
                if (id) autoRecalculate(id, date).then(() => reload().catch(() => {}));
                else reload().catch(() => {});
              }}
            />
          </div>
