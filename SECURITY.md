...
| Dependency scanning | ⚠️ Automated + manual | Dependabot is enabled for npm, pip, Docker, and GitHub Actions (`.github/dependabot.yml`). Continue running `npm audit` and `pip-audit` periodically. |
...
| Database exposure | ✅ Internal only | PostgreSQL is not exposed to the host by default. The `db` service is internal to the Docker network. Add a `ports:` mapping only for local development if direct DB access is needed. |
| Yahoo Finance access | ⚠️ Unauthenticated | NAV sync uses `yfinance` without an API key. No credentials are at risk, but this may break if Yahoo restricts unauthenticated access. |
...
- Pin base image versions to specific digests (`postgres:18-alpine@sha256:...`)
  rather than floating tags to prevent supply-chain surprises.
- Do not add `ports:` to the `db` service in production — direct database
  access from the host is not needed when using Docker Compose networking.
