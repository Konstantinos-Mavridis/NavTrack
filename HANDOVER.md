...
| Background worker | Python 3.14, APScheduler, psycopg2-binary, yfinance |
...
Browser
  ↓
nginx frontend container (:8080 inside container, :3000 on host)
...
The worker's daily valuation job (23:05 Athens) performs the same calculation but only logs the result to stdout — it does not persist to a separate table.
...
| `portfolio_positions` | Materialised current holdings rebuilt from ledger |
...
- `API` (single-instrument sync endpoint)
- `API_ALL` (all-instruments sync endpoint)
- `API_ALL_FORCE` (all-instruments force-history sync endpoint)
- `WORKER_STARTUP`
- `SCHEDULER_AFTERNOON`
- `SCHEDULER_EVENING`
...
- View per-instrument sync history in the UI
...
- **SYNC_ON_STARTUP** env var (default true) controls whether worker startup triggers a full incremental NAV sync
...
- **Daily valuation job** — logs P&L at 23:05 Athens time (after the evening NAV sync completes) but does not persist results to a dedicated table. Results are stdout-only (visible via `docker compose -f compose.dev.yml logs -f worker`).
...
3. Open any instrument detail page and confirm Sync History entries appear
